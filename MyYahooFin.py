...
def get_quote_table(ticker , dict_result = True, headers = {'User-agent': 'Mozilla/5.0'}): 
    ...
    new_row=pd.DataFrame(tables[1])
    data = pd.concat([new_row,tables[0]]).reset_index(drop=True)
    ...
    new_row1=pd.DataFrame(quote_price)
    data = pd.concat([new_row1,data]).reset_index(drop=True)
...
def get_stats(ticker, headers = {'User-agent': 'Mozilla/5.0'}):
    ...
    table = tables[0]
    for elt in tables[1:]:
        new_row2=pd.DataFrame(elt)
        table = pd.concat([new_row2,table]).reset_index(drop=True)
